...
class PotentialField:
    ...
    def mark_NF1(self, goal):
        bitmap = copy.deepcopy(globVar.obstacles_bitmap)
    ...
    def mark_NF2(self, goal):
        ...
        for i in range(128):
            for j in range(128):
                if globVar.obstacles_bitmap[i][j] == 255 or i == 0 or i == 127 or j == 0 or j == 127:
                    for neighbor in utils.findNeighbors((i, j)):
                        if globVar.obstacles_bitmap[neighbor[0]][neighbor[1]] == 254:
                            distance[i][j] = 0
                            origin[(i, j)] = (i, j)
        ...
        while True:
            ...
            for q in Li:
                for neighbor in utils.findNeighbors(q):
                    if globVar.obstacles_bitmap[neighbor[0]][neighbor[1]] == 254 and bitmap[neighbor[0]][neighbor[1]] == 254:
